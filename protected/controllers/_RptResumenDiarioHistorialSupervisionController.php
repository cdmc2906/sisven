 ...
 * @author 
 */
class RptResumenDiarioHistorialSupervisionController extends Controller {
    ...
    public function actionIndex() {
        ...
        if (Yii::app()->request->isAjaxRequest) {
            ...
            Yii::app()->session['RptResumenDiarioHistorialSupervisionForm'] = '';
            $model = new RptResumenDiarioHistorialSupervisionForm();
//            var_dump($model);die();
            $this->render('/historialmb/rptResumenDiarioHistorialSupervision', array('model' => $model));
        }
    }
...
//           var_dump($_POST['RptResumenDiarioHistorialSupervisionForm']);die();
            if (isset($_POST['RptResumenDiarioHistorialSupervisionForm'])) {
                $model->attributes = $_POST['RptResumenDiarioHistorialSupervisionForm'];
//                var_dump($model);die();
//                Yii::app()->session['ModelForm'] = $model;
                Yii::app()->session['ModelForm'] = $model;

                if ($model->validate()) {
                    $supervisor = EjecutivoModel::model()->findAllByAttributes(array('e_usr_mobilvendor' => $model->supervisor));
//                    VAR_DUM
                    $rsRuta = new FRutaModel ();
                    $codEjecutivo = $model->ejecutivoSupervisar;
                    $ejecutivo = EjecutivoModel::model()->findAllByAttributes(array('e_usr_mobilvendor' => $codEjecutivo));
                    $diaGestionSupervisor = date("w", strtotime($model->fechagestion));
                    $ruta_dia_gestion = $model->rutaSupervisar . '-' . $ejecutivo[0]['e_iniciales'];
                    $visitasRutaEjecutivo = intval($rsRuta->getVisitasSupervisorRutaEjecutivoxFecha($ruta_dia_gestion, $model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['CLIENTESVISITADOS']);
                    ...
                    if ($visitasRutaEjecutivo > 0) {
                        ...
                        $historial = $fHistorial->getHistorialxVendedorxFechaxHoraInicioxHoraFin($model->accionHistorial,$model->fechagestion, $model->horaInicioGestion, $model->horaFinGestion, $supervisor[0]['e_usr_mobilvendor']);

                        if (count($historial)) {

                            $primeraVisita = $fHistorial->getPrimeraVisitaxEjecutivoxFechaxHoraInicioxHoraFin($model->accionHistorial,$model->fechagestion, $model->horaInicioGestion, $model->horaFinGestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESULTADO'];
                            $ultimaVisita = $fHistorial->getUltimaVisitaxEjecutivoxFechaxHoraInicioxHoraFin($model->accionHistorial,$model->fechagestion, $model->horaInicioGestion, $model->horaFinGestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESULTADO'];
...
//                            $ruta_dia_gestion = "R" . $diaGestion . '-' . $ejecutivo[0]['e_iniciales'];
                            ...
                            $clientesNoVisitados = $fRuta->getTotalClientesNoVisitadosxRutaxEjecutivo($ejecutivo[0]['e_iniciales'], $diaGestion + 1, $model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['CLIENTESNOVISITADOS'];
                            $visitasRuta = 0;
...
//                        var_dump($mesGestion,$mesAnterior);die();
                            ...
                            if ($mesAnterior == $mesGestion - 1) {
                                ...
                                if ($diaGestion == 1) {//1--> Lunes
                                    ...
                                    $_totalVentaViernes = intval($rsTotales->getTotalChipsVentaDia($fechaViernes, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaRutaViernes = intval($rsTotales->getTotalChipsVentaRuta($ejecutivo[0]['e_iniciales'], 6, $fechaViernes, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaFueraRutaViernes = intval($rsTotales->getTotalChipsVentaFueraRuta($ejecutivo[0]['e_iniciales'], 6, $fechaViernes, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalClientesVentaViernes = intval($rsTotales->getTotalClientesVenta($fechaViernes, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);

                                    $_totalVentaFinSemana = intval($rsTotales->getTotalChipsVentaFinSemana($fechaViernes, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaRutaFinSemana = intval($rsTotales->getTotalChipsVentaFinSemana($fechaViernes, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalClientesVentaFinSemana = intval($rsTotales->getTotalClientesVentaFinSemana($fechaViernes, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);

                                    $_totalVentaDiaLunes = intval($rsTotales->getTotalChipsVentaDia($model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaRutaLunes = intval($rsTotales->getTotalChipsVentaRuta($ejecutivo[0]['e_iniciales'], $diaGestion + 1, $model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaFueraRutaLunes = intval($rsTotales->getTotalChipsVentaFueraRuta($ejecutivo[0]['e_iniciales'], $diaGestion + 1, $model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalClientesVentaLunes = intval($rsTotales->getTotalClientesVenta($model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    ...
                                    $_totalVentaDiaAnterior = intval($rsTotales->getTotalChipsVentaDia($fechaDiaAnterior, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaRutaDiaAnterior = intval($rsTotales->getTotalChipsVentaRuta($ejecutivo[0]['e_iniciales'], $diaGestion, $fechaDiaAnterior, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaFueraRutaDiaAnterior = intval($rsTotales->getTotalChipsVentaFueraRuta($ejecutivo[0]['e_iniciales'], $diaGestion, $fechaDiaAnterior, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalClientesVentaDiaAnterior = intval($rsTotales->getTotalClientesVenta($fechaDiaAnterior, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);

                                    $_totalVentaDiaE = intval($rsTotales->getTotalChipsVentaDia($model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaRutaDia = intval($rsTotales->getTotalChipsVentaRuta($ejecutivo[0]['e_iniciales'], $diaGestion + 1, $model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaFueraRutaDia = intval($rsTotales->getTotalChipsVentaFueraRuta($ejecutivo[0]['e_iniciales'], $diaGestion + 1, $model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalClientesVentaDia = intval($rsTotales->getTotalClientesVenta($model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
...
//                                var_dump($_totalVentaDia,$_totalVentaRuta,$_totalVentaFueraRuta,$_totalClientesVenta);die();
                                ...
                                if ($diaGestion == 5) { //5-> viernes
                                    $_totalVentaViernes = intval($rsTotales->getTotalChipsVentaDia($model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaRutaViernes = intval($rsTotales->getTotalChipsVentaRuta($ejecutivo[0]['e_iniciales'], $diaGestion + 1, $model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaFueraRutaViernes = intval($rsTotales->getTotalChipsVentaFueraRuta($ejecutivo[0]['e_iniciales'], $diaGestion + 1, $model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalClientesVentaViernes = intval($rsTotales->getTotalClientesVenta($model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);

                                    $_totalVentaFinSemana = intval($rsTotales->getTotalChipsVentaFinSemana($ejecutivo[0]['e_iniciales'], $diaGestion + 1, $model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaRutaFinSemana = intval($rsTotales->getTotalChipsVentaFinSemana($ejecutivo[0]['e_iniciales'], $diaGestion + 1, $model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    ...
                                    $_totalVentaDia = intval($rsTotales->getTotalChipsVentaDia($model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaRuta = intval($rsTotales->getTotalChipsVentaRuta($ejecutivo[0]['e_iniciales'], $diaGestion + 1, $model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalVentaFueraRuta = intval($rsTotales->getTotalChipsVentaFueraRuta($ejecutivo[0]['e_iniciales'], $diaGestion + 1, $model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                    $_totalClientesVenta = intval($rsTotales->getTotalClientesVenta($model->fechagestion, $supervisor[0]['e_usr_mobilvendor'])[0]['RESPUESTA']);
                                }
                            }
                            $visitasValidas = 0;
                            $visitasInvalidas = 0;
                            ...
                            foreach ($historial as $itemHistorial) {
                                ...
                                $estadoRevisionRuta = '';
                                $cantidadChips = $fOrden->getChipsxClientexEjecutivoxFecha($itemHistorial['CODIGOCLIENTE'], $supervisor[0]['e_usr_mobilvendor'], $fechaHistorial);
...
//                $response->Result = $datos; // $datosGrid;
                    ...
                    $response->Message = "Debe seleccionar todos los filtros marcados como obligatorios (*)";
                    $response->ClassMessage = CLASS_MENSAJE_NOTICE;
//                $response->Result = $datos; // $datosGrid;
                }
            }
    ...
    public function actionGuardarRevision() {
        ...
        try {
            if (isset(Yii::app()->session['detallerevisionhistorialitem']) && isset(Yii::app()->session['resumenrevisionhistorialitem'])) {
                ...
                $comentarioSupervisor = $_POST['RptResumenDiarioHistorialSupervisionForm']['comentarioSupervision'];
                $enlaceMapa = $_POST['RptResumenDiarioHistorialSupervisionForm']['enlaceMapa'];

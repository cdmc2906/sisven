...
class ModelNotifyii extends CActiveRecord
{
    ...
    public function rules()
    {
        return array(
            ...
            array('id, expire, alert_after_date, alert_before_date, content, role, link, title, iduser', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function attributeLabels()
    {
        return array(
            ...
            'title' => 'Titulo',
            'expire' => 'Expira',
            'alert_after_date' => 'Alerta Despues de Fecha',
            'alert_before_date' => 'Alerta antes de la fecha',
            'content' => 'Contenido',
            'role' => 'Rol',
            'link' => 'Enlace',
    ...
    public static function getAllUserNotifications($iduser = '1')
    {
        $criteria = new CDbCriteria(array(
                    'condition' => ':now >= t.alert_after_date AND :now <= t.alert_before_date AND t.iduser = :iduser',
                    'params' => array(
                        ':now' => date('Y-m-d'),
                        ':iduser' => $iduser,
                    )

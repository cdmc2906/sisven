...
class CrugeMailer extends CrugeMailerBase implements ICrugeMailer {
    ...
    public function enviarReporteAltasFueraZona($to, $model, $pathArchivoAdjunto) {
        $this->sendEmail(
                $to
                , 'Reporte Altas Fuera Zona'
                , $this->render(
                        'envioAltasFueraZonaxEjecutivo'
                        , array('model' => $model)
                ), $pathArchivoAdjunto
        );
    }
    ...
    public function sendEmail($to, $subject, $body, $adjunto) {
//        var_dump($body);die();
        return parent::sendEmail($to, $subject, $body, $adjunto);
    }

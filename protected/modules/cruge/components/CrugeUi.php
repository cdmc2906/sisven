...
class CrugeUi extends CComponent {
    ...
    public function getRbacAuthItemCreateUrl($type, $extra = '') {
        ...
        return CrugeUtil::uiaction(
                        'rbacauthitemcreate', array('type' => $type, 'extra' => $extra)
        );
    }
    ...
    public function addError($itemName, $tipoItemName = '', $description = "") {
        if (CrugeUtil::config()->rbacSetupEnabled == true) {
            CrugeUtil::config()->globalErrors[] = array('itemName' => $itemName, 'type' => $tipoItemName, 'descr' => $description);
        }
            ...
            foreach (CrugeUtil::config()->globalErrors as $gerr) {
                if ($n == 0) {
                    ...
                    $title = CrugeTranslator::t(
                                    'logger', 'This page displays the roles, tasks and operations that are required by the current user but unassigned. This message is displayed because CrugeModule::rbacSetupEnabled = true'
                    );
                    $icon = "";
    ...
    public function setupAlert($message) {
        ...
        $alertReason = CrugeTranslator::t(
                        'admin', 'This message is displayed because you have "debug" parameter enabled in the config file.'
        );

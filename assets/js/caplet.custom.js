$(function() {
    ...
    $("a.im-confirm").click(function() {
        ...
        if (confirm == "yes" || confirm == "accept") {
            // Ajax para guardar el registro de la lectura de la notificacion
            var data = $(this).data();
            $.ajax({
                url: data.url,
                cache: false,
...
//    });
    ...
    $('.knob').each(function() {
        ...
        $data.fgColor = $.fillColor(thisKnob) || "#BD3B47";
        thisKnob.knob($data);
        if ($data.animate) {
    ...
    $(".md-effect").click(function(event) {
        ...
        var data = $(this).data();
        $("#" + data.id).attr('class', 'modal fade').addClass(data.effect).modal('show');
    });
